...
/* Description: This project focuses on customer segmentation using SQL to analyze purchasing behavior. 
				By applying RFM (Recency, Frequency, Monetary) analysis, customers are grouped into meaningful segments. 
...
BEGIN
	...
	before_analysis_date_orders AS (
		SELECT customer_id, MAX(order_date) AS last_order_before_analysis_date
        FROM cleaned_orders
        WHERE order_date < @analysis_date
        GROUP BY customer_id
	),
    RFM_table AS (
        SELECT 
            c.customer_id, 
            CASE 
                WHEN a.last_order_after_analysis_date IS NULL THEN ABS(DATEDIFF(DAY, b.last_order_before_analysis_date, '2025-04-15'))
                ELSE ABS(DATEDIFF(DAY, a.last_order_after_analysis_date, '2025-04-15'))
            END AS Recency,
		...
		LEFT JOIN before_analysis_date_orders b ON c.customer_id = b.customer_id
        GROUP BY c.customer_id, a.last_order_after_analysis_date, b.last_order_before_analysis_date
    ),
	...
	RFM_w_recency AS (
        ...
        FROM RFM_table
		WHERE Recency <= DATEDIFF(DAY, @analysis_date, '2025-04-15')
	),
	RFM_percentile AS (
		SELECT
			wo.customer_id,
			...
			CASE
				WHEN w.percent_rank_recency IS NULL THEN t.Recency
				ELSE w.percent_rank_recency
			END AS percent_rank_recency
		FROM RFM_percentile_WO_recency AS wo
		LEFT JOIN RFM_w_recency AS w
			ON wo.customer_id = w.customer_id
		LEFT JOIN RFM_table t 
			ON wo.customer_id = t.customer_id
	),
    ...
    RFM_Label AS (
        SELECT 
            ...
            Monetary,
            CASE	
				WHEN percent_rank_recency > DATEDIFF(DAY, @analysis_date, '2025-04-15') THEN 4 ---Used to order before analysis date but does not order anymore
                WHEN percent_rank_recency < 0.25 and percent_rank_recency >= 0 THEN 1 
